...
| 2026-06 | mobile-responsive + cache-fix | (a) **Stale-asset cache fix** (root cause of the 'View All Cases narrow layout still there'): now that the app ships a separate `app.js`, Flask was telling the browser to cache it for 12 h, so deploys weren't picked up. Set `SEND_FILE_MAX_AGE_DEFAULT = 0` + `Cache-Control: no-cache` on the HTML shell so the browser revalidates every load (cheap 304 when unchanged, instant pickup when changed). (b) **Full mobile pass**: added a `.form-grid` class to all 12 fixed 2-/3-column form rows (Add Case, edit, renew, partial-payment, auth modal, profile edit, plan tiles) with a `@media (max-width:560px){ .form-grid{grid-template-columns:1fr} }` rule so they stack to one column on phones while staying multi-column on desktop. Navbar title shrinks at ≤560px and hides at ≤380px (logo only). Tables get `-webkit-overflow-scrolling: touch`. Verified in headless Chrome at 390px (1 col, no h-overflow) and 1440px (2 col). The ViewCasesPage `width:100%` fix from before was already correct — confirmed empirically (grid renders 6 cols at 1114px); the lingering symptom was purely the cached old app.js. |

| 2026-06 | profile-open-perf | Fixed the 2-3 s lag when opening the profile panel (and modals). Root cause: every full-page overlay used `backdrop-filter: blur` — blurring the ENTIRE viewport (tables, charts, everything behind) on each frame of the slide-in animation is very GPU-expensive and stalls the main thread. Confirmed the 3 API calls the panel fires are only ~0.6 s in parallel, so the freeze was pure compositing cost. Removed `backdrop-filter` from all four full-page overlays (ProfilePanel, AuthModal, generic Modal, CaseDetailModal) and bumped the solid overlay opacity slightly to keep the dimmed look. Kept the cheap small-element blurs (landing nav bar, hero glass card, hero CTA). Profile now opens instantly; only the body content waits ~0.6 s for data. |

_Last updated: 2026-06-01 — commit `profile-open-perf`_
